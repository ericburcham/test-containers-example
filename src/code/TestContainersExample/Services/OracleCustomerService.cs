...
public sealed class OracleCustomerService : CustomerService, ICustomerService
{
    private readonly IProvideDbConnections _connectionProvider;

    public OracleCustomerService(IProvideDbConnections connectionProvider)
    {
    ...
    public override void CreateCustomersTable()
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText =
            "CREATE TABLE Customers (Id NUMBER(19) NOT NULL, Name VARCHAR2(100) NOT NULL, PRIMARY KEY (Id))";
        command.Connection!.Open();
        command.ExecuteNonQuery();
    }
}